...
app.config["SQLALCHEMY_DATABASE_URI"] = (
    "postgresql://postgres:andrei@localhost:5432/helping_hand_3"
)
...
class User(UserMixin, db.Model):
    ...
    feedbacks_given = db.relationship(
        "Feedback", backref="reviewer", lazy=True, foreign_keys="Feedback.reviewer_id"
    )
    ...
    interests_sent = db.relationship(
        "Interest",
        backref="interested_user",
        lazy=True,
        foreign_keys="Interest.user_id",
    )
    ...
    def services_offered(self):
        return Listing.query.filter_by(
            user_id=self.id, category="Skills & Services", listing_type="Offering"
        ).all()

    @property
    def services_needed(self):
        return Listing.query.filter_by(
            user_id=self.id, category="Skills & Services", listing_type="Requesting"
        ).all()

    @property
    def items_offered(self):
        return Listing.query.filter_by(
            user_id=self.id, category="Items & Resources", listing_type="Offering"
        ).all()

    @property
    def items_needed(self):
        return Listing.query.filter_by(
            user_id=self.id, category="Items & Resources", listing_type="Requesting"
        ).all()
...
class Listing(db.Model):
    ...
    feedbacks = db.relationship(
        "Feedback", backref="listing", lazy=True, cascade="all, delete-orphan"
    )
    interests = db.relationship(
        "Interest", backref="listing", lazy=True, cascade="all, delete-orphan"
    )
...
def create_notification(
    recipient_id, sender_id, listing_id, notification_type, message
):
...
def create_admin_accounts():
    ...
    ADMIN_ACCOUNTS = [
        ...
        {"username": "janrey", "password": "database123"},
    ]
    ...
    if request.method == "POST":
        ...
        if user and user.password == password:
            ...
            if not user.is_profile_complete:
                flash(
                    f"Welcome, {user.username}!  Please complete your profile to continue.",
                    "success",
                )
                return redirect(url_for("complete_profile"))
...
def complete_profile():
    ...
    if request.method == "POST":
        ...
        db.session.commit()
        flash(
            "Profile completed successfully!  Welcome to The Helping Hand! ", "success"
        )
        return redirect(url_for("home"))
...
def listing_detail(id):
    ...
    if current_user.is_authenticated and not current_user.is_admin:
        existing_feedback = Feedback.query.filter_by(
            listing_id=listing.id, reviewer_id=current_user.id
        ).first()
        user_has_reviewed = existing_feedback is not None

        existing_interest = Interest.query.filter_by(
            listing_id=listing.id, user_id=current_user.id
        ).first()
...
def create_listing():
    ...
    if request.method == "POST":
        ...
        db.session.commit()
        flash(
            "Listing created successfully!  It will now appear on your profile.",
            "success",
        )
        return redirect(url_for("listings"))
...
def edit_listing(id):
    ...
    if listing.has_feedback:
        flash(
            "This listing cannot be edited because it has received feedback.  You can only delete it.",
            "error",
        )
        return redirect(url_for("listing_detail", id=id))
...
def view_interests(id):
    ...
    interests = (
        Interest.query.filter_by(listing_id=listing.id)
...
def admin_dashboard():
    ...
    all_notifications = (
        Notification.query.order_by(Notification.created_at.desc()).limit(50).all()
    )
